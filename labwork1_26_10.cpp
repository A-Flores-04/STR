...
void cylinder1(void* pvParameters) {
	...
	xQueueHandle dock_to_two = my_params->dock_to_two;
	...
	while (TRUE) {
		...
		if (getBitValue(readDigitalU8(0), 0)) {
			...
			if (dock == 1) {
				...
				xQueueSend(dock_to_two, &dock, 10);
				brickType = 0;
				dock = 0;
			}
			if (brickType == 1) {
...
void cylinder2(void* pvParameters) {
	...
	while (TRUE) {
		...
		if (getBitValue(readDigitalU8(1), 7)) {
			...
			xQueueReceive(dock_to_two, &dock, 10);
...
void myDaemonTaskStartupHook(void) {
	...
	xQueueHandle dock_to_two = xQueueCreate(10, sizeof(int));
	...
	cylinder1_Params->dock_to_two = dock_to_two;
	...
	cylinder2_Params->dock_to_two = dock_to_two;
